...
class SampleSheetReader(object):
    ...
    def __init__(self, filename, sep=','):
        ...
        self.sep_count = self._val_count_cols(filename, sep)  # Check to make sure all rows have same number of columns.
        ...
        self._data_header = self._create_data_header(self.data)
    ...
    def _create_data_header(self, data_dict):
        """
        Return a list that is the header of the Data section.
        """

        try:
            data_header = data_dict['Sample_ID'][0]
        except:
    ...
    def _reformat_data_section(self, data_dict, data_header):
        ...
        for key in data_dict:
            if key != data_header[0]:
                sample_dict[key] = collections.OrderedDict({data_header[0]: data_dict[key][0][0]})
    ...
    def _val_count_cols(self, filename, sep):
        """
...
class SampleSheetWriter(object):
    ...
    def __init__(self, samplesheet, filename, sep_count, sep=','):

        self.samplesheet = samplesheet
        self.filename = filename
        self.sep = sep

        self.write_sheet(filename, samplesheet, sep, sep_count)
